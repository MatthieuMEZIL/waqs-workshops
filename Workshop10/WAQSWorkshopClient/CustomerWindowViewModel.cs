...
namespace WAQSWorkshopClient
{
    public class CustomerWindowViewModel : ViewModelBase
    {
        ...
        private async Task SaveChangesAsync()
        {
            ...
            {
                System.Windows.MessageBox.Show(String.Join(Environment.NewLine, e.Errors.OrderByDescending(er => er.Criticity).Select(er => (er.ErrorInfo == null ? "" : (er.ErrorInfo.PropertyName + ": ")) + er.Message))); // this is not good for an MVVM point of view
            }
            catch (Exception e)
            {

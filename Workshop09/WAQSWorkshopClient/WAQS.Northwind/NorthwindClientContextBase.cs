...
namespace WAQSWorkshopClient.ClientContext
{
    public abstract partial class NorthwindClientContextBase : IClientContextBase
    {
            ...
            getEntity = (value, type) =>
                {
                    ...
                    var attachedDtosCustomerInfo = new List<WAQSWorkshopClient.DTO.CustomerInfo>();
                    var attachedDtosLastOrderDTO = new List<WAQSWorkshopClient.DTO.LastOrderDTO>();
                    WAQSWorkshopClient.DTO.CustomerInfo valueAsCustomerInfo;
                    if ((valueAsCustomerInfo = (value as WAQSWorkshopClient.DTO.CustomerInfo)) != null)
                        return GetDTO(valueAsCustomerInfo, attachedDtosCustomerInfo, attachedDtosLastOrderDTO, mergeOption);
                    var valueAsCustomerInfos = value as IEnumerable<WAQSWorkshopClient.DTO.CustomerInfo>;
                    if (valueAsCustomerInfos != null)
                    {
                        var ctor = value.GetType().GetConstructor(new Type[] { typeof(IEnumerable<WAQSWorkshopClient.DTO.CustomerInfo>) });
                        if (ctor != null)
                            return ctor.Invoke(new object[] { valueAsCustomerInfos.Select(e => GetDTO(e, attachedDtosCustomerInfo, attachedDtosLastOrderDTO, mergeOption)) });
                        valueAsCustomerInfos = valueAsCustomerInfos.Select(e => GetDTO(e, attachedDtosCustomerInfo, attachedDtosLastOrderDTO, mergeOption));
                        if (type.IsArray)
                            return valueAsCustomerInfos.ToArray();
                        return valueAsCustomerInfos.ToList();
                    }
                    WAQSWorkshopClient.DTO.LastOrderDTO valueAsLastOrderDTO;
                    if ((valueAsLastOrderDTO = (value as WAQSWorkshopClient.DTO.LastOrderDTO)) != null)
                        return GetDTO(valueAsLastOrderDTO, attachedDtosCustomerInfo, attachedDtosLastOrderDTO, mergeOption);
                    var valueAsLastOrderDTOs = value as IEnumerable<WAQSWorkshopClient.DTO.LastOrderDTO>;
                    if (valueAsLastOrderDTOs != null)
                    {
                        var ctor = value.GetType().GetConstructor(new Type[] { typeof(IEnumerable<WAQSWorkshopClient.DTO.LastOrderDTO>) });
                        if (ctor != null)
                            return ctor.Invoke(new object[] { valueAsLastOrderDTOs.Select(e => GetDTO(e, attachedDtosCustomerInfo, attachedDtosLastOrderDTO, mergeOption)) });
                        valueAsLastOrderDTOs = valueAsLastOrderDTOs.Select(e => GetDTO(e, attachedDtosCustomerInfo, attachedDtosLastOrderDTO, mergeOption));
                        if (type.IsArray)
                            return valueAsLastOrderDTOs.ToArray();
                        return valueAsLastOrderDTOs.ToList();
                    }
        ...
        protected abstract WAQSWorkshopClient.DTO.CustomerInfo GetDTO(WAQSWorkshopClient.DTO.CustomerInfo dto, List<WAQSWorkshopClient.DTO.CustomerInfo> attachedDtosCustomerInfo, List<WAQSWorkshopClient.DTO.LastOrderDTO> attachedDtosLastOrderDTO, MergeOption? mergeOption = null, bool applyDataTransfer = false);
    
        protected abstract WAQSWorkshopClient.DTO.LastOrderDTO GetDTO(WAQSWorkshopClient.DTO.LastOrderDTO dto, List<WAQSWorkshopClient.DTO.CustomerInfo> attachedDtosCustomerInfo, List<WAQSWorkshopClient.DTO.LastOrderDTO> attachedDtosLastOrderDTO, MergeOption? mergeOption = null, bool applyDataTransfer = false);

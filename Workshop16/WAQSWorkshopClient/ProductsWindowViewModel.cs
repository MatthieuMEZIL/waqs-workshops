...
namespace WAQSWorkshopClient
{
    public class ProductsWindowViewModel : ViewModelBase
    {
        ...
        public string FullNameSearch
        {
            ...
            set
            {
                var previousFullNameSearch = _fullNameSearch;
                _fullNameSearch = value;
                NotifyPropertyChanged.RaisePropertyChanged(nameof(FullNameSearch));
                if (!string.IsNullOrEmpty(previousFullNameSearch) && value != null && value.StartsWith(previousFullNameSearch) && ProductsByStepQuery.IsCompleted)
                {
                    for (int i = 0; i < ProductsByStepQuery.Items.Count;)
                    {
                        var p = ProductsByStepQuery.Items[i];
                        if (!p.FullName.Contains(_fullNameSearch))
                        {
                            ProductsByStepQuery.Items.RemoveAt(i);
                        }
                        else
                        {
                            i++;
